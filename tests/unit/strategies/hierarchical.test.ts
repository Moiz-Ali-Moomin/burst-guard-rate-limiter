import { HierarchicalRateLimitStrategy } from '../../../src/strategies/hierarchical/hierarchicalRateLimitStrategy';
import { RateLimitStrategy } from '../../../src/strategies/base';

const mockEvalsha = jest.fn();
const mockScript = jest.fn().mockResolvedValue('sha-hierarchical');
...
describe('HierarchicalRateLimitStrategy', () => {
  let strategy: HierarchicalRateLimitStrategy;

  beforeEach(() => {
    jest.clearAllMocks();
    strategy = new HierarchicalRateLimitStrategy();
  });

  it('has the correct strategy name', () => {
    expect(strategy.name).toBe('hierarchical' as RateLimitStrategy);
  });
