...
import { LUA_HIERARCHICAL_RATE_LIMIT } from '../../redis/advancedLuaScripts';
import { logger } from '../../utils/logger';

export class HierarchicalRateLimitStrategy implements IRateLimitStrategy {
  readonly name = 'hierarchical' as RateLimitStrategy;
  private scriptSha: string | null = null;

  private async loadScript(): Promise<string> {
    if (this.scriptSha) return this.scriptSha;
    const client = getRedisClient();
    this.scriptSha = (await client.script('LOAD', LUA_HIERARCHICAL_RATE_LIMIT)) as string;
  ...
  private async _exec(options: RateLimitOptions, retried: boolean): Promise<RateLimitResult> {
    ...
    try {
      ...
      logger.error({ err, key }, 'HierarchicalRateLimitStrategy error');
      throw err;
    }
  }
}
