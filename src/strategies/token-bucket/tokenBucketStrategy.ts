...
export class TokenBucketStrategy implements IRateLimitStrategy {
  ...
  private async _exec(options: RateLimitOptions, retried: boolean): Promise<RateLimitResult> {
    const { key, limit, windowMs, burstLimit, refillRate } = options;
    const now = Date.now();
    const capacity = burstLimit ?? limit;
    const rate = refillRate ?? limit / (windowMs / 1000);
    const client = getRedisClient();

    try {
      const sha = await this.loadScript();
      const result = (await client.evalsha(sha, 1, key, capacity, rate, now, 1, windowMs * 2)) as [
        number,
        number,
        number,
      ];
      ...
      logger.error({ err, key }, 'TokenBucketStrategy error');
      throw err;
    }
  }
}
