...
export class FixedWindowStrategy implements IRateLimitStrategy {
  ...
  private async _exec(options: RateLimitOptions, retried: boolean): Promise<RateLimitResult> {
    ...
    try {
      ...
      logger.error({ err, key }, 'FixedWindowStrategy error');
      throw err;
    }
  }
}
